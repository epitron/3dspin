...
void Banner(void) {
    printf("\n3DSRDR, 3D Studio formats reader v1.2 by Javier Arevalo AKA Jare/Iguana.\n"
           "Modified by Mats Byggmastar aka. MRI/Doomsday (v1.3)\n"
           "Slightly Modified by Chris Gahan for use with 3DSpinner\n");
}
...
void main(int argc, char *argv[]) {
    ...
    if (argc < 2) {
        ...
        printf("Usage: 3DSRDR file.3DS (or .PRJ or .MLI) [-quiet, -verbose or -dump]\n\n"
               "NOTE: file.3DS will automatically be translated into 3DSpinner MESH format and\n"
               "      written as 'file.MSH' in the same directory as file.3DS\n");
        exit(1);
    }
